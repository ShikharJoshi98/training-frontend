...
const CourseModal = ({ onClose }: any) => {
    const { courseCategories, getCourseCategory, courses, getCourses } = useStore();
    const [selectCourse, setSelectedCourse] = useState<string>("");

    useEffect(() => {
        getCourseCategory();
        getCourses();
    }, [getCourseCategory, getCourses]);
                ...
                <div className="flex md:flex-row flex-col gap-3 md:gap-0 md:justify-center h-[90%]">
                    ...
                    <div className="bg-white w-full md:w-[72%] flex flex-wrap gap-4 overflow-x-auto p-4 rounded-md md:rounded-l-none md:rounded-r-md">
                        {courses.filter((course, _) => course?.courseCategory === selectCourse).map((course, index) => (
                            <Link href={`/Courses/${(course?.course).replace(/\s+/g, "-")}/${course?.id}`} onClick={() => onClose()} key={index} className="p-2.5 shadow-md shadow-gray-300 cursor-pointer group hover:bg-gray-100 duration-300 w-[180px] h-[180px] min-h-48 flex flex-col items-center gap-3 rounded-md">
                                <div className="p-4 rounded-full group-hover:bg-white bg-gray-100">
                                    <Image src={course?.courseLogo} alt={course?.course} height={60} width={60} />
                                </div>
